...
from .nodes import analyzing_cos_sim_between_task_and_nl_desc, generate_boxplot_from_cos_sim_data

def create_pipeline(**kwargs) -> Pipeline:
    return pipeline([
        node(
            func=analyzing_cos_sim_between_task_and_nl_desc,
            inputs=[
                "cross_encoder_model",
                "params:setup_sentence_encoder_cfg",
            ],
            outputs="cosine_sim_comparison_data#csv",
            name="analyzing_cos_sim_between_task_and_nl_desc_node"
        ),
        node(
